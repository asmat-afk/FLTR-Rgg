...
def run_simulation_parallel(params):
    ...
    with open('data/keys{}.txt'.format(params.n), 'r') as f:
        # original radius
        radius = eval(f.read())
    # pick the radius of interest
    r = radius[params.r]
    del radius
    # load resistance values
    res = np.load('data/res_phase1.npy')
    # load graphs G(n,r_i) from adjacency matrices
    matrices = np.load('data/graphs/graph_{}_{}.npy'.format(params.n, r))
    ...
    del out
    raw_data.to_csv('data/maxpred/data_{}_{}.csv'.format(params.n, r))
    ...
    data_per_node.to_csv('data/maxpred/data_node_{}_{}.csv'.format(params.n, r))
    del data_per_node
    ...
    data_per_prob.to_csv('data/maxpred/data_graph_{}_{}.csv'.format(params.n, r))
    del data_per_prob
    del raw_data
